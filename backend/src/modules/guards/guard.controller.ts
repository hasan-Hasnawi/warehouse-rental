...
export async function createReport(req: AuthRequest, res: Response) {
  try {
    ...
    const report = await prisma.guardReport.create({
      data: { ...data, images: JSON.stringify(data.images), guardId: req.user!.id },
    });
    ...
    if (data.severity === 'emergency') {
      ...
      await prisma.notification.createMany({
        data: admins.map(a => ({
          ...
          message: `${data.title}: ${data.description.substring(0, 100)}`,
          type: 'emergency',
        })),
      });
    }
...
export async function getReports(req: AuthRequest, res: Response) {
  try {
    ...
    const reports = await prisma.guardReport.findMany({
      where,
...
export async function createCollection(req: AuthRequest, res: Response) {
  try {
    ...
    const collection = await prisma.guardCollection.create({
      data: { ...data, guardId: req.user!.id },
    });
...
export async function getCollections(req: AuthRequest, res: Response) {
  try {
    ...
    const collections = await prisma.guardCollection.findMany({
      where,
      include: { guard: { select: { fullName: true } }, client: { select: { fullName: true } } },
...
export async function generateQR(req: AuthRequest, res: Response) {
  try {
    const contract = await prisma.contract.findUnique({
      ...
      include: { client: { select: { fullName: true } }, warehouse: { select: { name: true, code: true } } },
    });
...
export async function createTask(req: AuthRequest, res: Response) {
  try {
    ...
    const task = await prisma.guardTask.create({
...
export async function getTasks(req: AuthRequest, res: Response) {
  try {
    ...
    const tasks = await prisma.guardTask.findMany({
      where,
      include: {
...
export async function updateTaskStatus(req: AuthRequest, res: Response) {
  try {
    ...
    const task = await prisma.guardTask.update({
